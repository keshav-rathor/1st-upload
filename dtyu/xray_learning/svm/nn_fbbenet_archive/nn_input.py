...
def read_data(filename_queue):
  ...
  result.coef = tf.mul(tf.sign(normalized_coef), tf.log(tf.abs(normalized_coef) + 1) / np.log(256.))
...
def _generate_image_and_label_batch(key, coef, image, label, min_queue_examples,
  ...
  if input_type is None:
    ...
    tf.image_summary('images_' + input_type, images, max_images=20)
  cs = coefs.get_shape()
  empty_dim = tf.zeros([cs[0].value, cs[1].value, cs[2].value, 1])
  summary_coefs = tf.concat(3, [coefs, empty_dim])
  if input_type is None:
    tf.image_summary('coefs', summary_coefs, max_images=20)
  else:
    tf.image_summary('coefs_' + input_type, summary_coefs, max_images=20)
...
def distorted_inputs(run_config, batch_size, shuffle=True, input_type=None, num_threads=16, nodistort=False):
  ...
  normalized_coef = tf.image.per_image_whitening(read_input.coef)
...
def inputs(run_config, batch_size, num_threads=16):
  ...
  # Subtract off the mean and divide by the variance of the pixels.
  normalized_image = tf.image.per_image_whitening(resized_image)
