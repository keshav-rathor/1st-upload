  ...
  """
    ...
    if architecture == 'cnn':
        ...
        with tf.variable_scope('fc2') as scope:
            ...
            fc2 = tf.nn.relu(fc2l)
            w_images.extend([fc2w, fc2b])

        # fc3
        with tf.variable_scope('fc3') as scope:
            fc3w = _variable_with_weight_decay('weights', shape=[4096, NUM_CLASSES],
                                               stddev=0.01, wd=0.0)
            fc3b = _variable_on_cpu('biases', [NUM_CLASSES], tf.constant_initializer(0.1))
            fc3l = tf.nn.bias_add(tf.matmul(fc2, fc3w), fc3b)
            w_images.extend([fc3w, fc3b])

            return fc3l, w_coefs, w_images, fc1
        ...
        with tf.variable_scope('fconv1') as scope:
            ...
            ffilter_layer1 = tf.split(0, ffilter_layer1_height, ffilter_layer1)
            for height_idx in range(0, ffilter_layer1_height):
                ffilter_layer1[height_idx] = tf.reshape(
                    ffilter_layer1[height_idx],
                    [ffilter_layer1_width, ffilter_w, ffilter_w, ffilter_d])
                tmp = tf.split(0, ffilter_layer1_width,
                               ffilter_layer1[height_idx])
                for width_idx in range(0, ffilter_layer1_width):
                    ...
                    tmp[width_idx] = tf.concat(2, [tmp[width_idx], empty_dim])
                ffilter_layer1[height_idx] = tf.concat(0, tmp)
            ffilter_layer1 = tf.concat(1, ffilter_layer1)
            # filter_layer1 = tf.reshape(filter_layer1, [1, filter_layer1_height*filter_w, filter_layer1_width*filter_w, 1])
            ffilter_layer1 = tf.pack([ffilter_layer1])
            tf.image_summary('ffilter1', ffilter_layer1)
        ...
        fpool2 = tf.nn.max_pool(fconv2, ksize=[1, 4, 4, 1],
                                strides=[1, 2, 2, 1],
        ...
        with tf.variable_scope('ffc1') as scope:
            ...
            biases = _variable_on_cpu('biases', [256],
                                      tf.constant_initializer(0.1))
            fc1 = tf.nn.relu(tf.matmul(reshape, weights) + biases,
                             name=scope.name)
        ...
        with tf.name_scope('conv1_1') as scope:
            kernel = tf.Variable(
                tf.truncated_normal([3, 3, 1, 64], dtype=tf.float32,
        ...
        with tf.name_scope('conv1_2') as scope:
            kernel = tf.Variable(
                tf.truncated_normal([3, 3, 64, 64], dtype=tf.float32,
                                    stddev=1e-1), name='weights')
            conv = tf.nn.conv2d(conv1_1, kernel, [1, 1, 1, 1], padding='SAME')
            biases = tf.Variable(tf.constant(0.0, shape=[64], dtype=tf.float32),
                                 trainable=True, name='biases')
        ...
        with tf.name_scope('conv2_1') as scope:
            kernel = tf.Variable(
                tf.truncated_normal([3, 3, 64, 128], dtype=tf.float32,
                                    stddev=1e-1), name='weights')
            conv = tf.nn.conv2d(pool1, kernel, [1, 1, 1, 1], padding='SAME')
            biases = tf.Variable(
        ...
        with tf.name_scope('conv2_2') as scope:
            kernel = tf.Variable(
                tf.truncated_normal([3, 3, 128, 128], dtype=tf.float32,
                                    stddev=1e-1), name='weights')
            conv = tf.nn.conv2d(conv2_1, kernel, [1, 1, 1, 1], padding='SAME')
            biases = tf.Variable(
                tf.constant(0.0, shape=[128], dtype=tf.float32),
        ...
        with tf.name_scope('conv3_1') as scope:
            kernel = tf.Variable(
                tf.truncated_normal([3, 3, 128, 256], dtype=tf.float32,
                                    stddev=1e-1), name='weights')
            conv = tf.nn.conv2d(pool2, kernel, [1, 1, 1, 1], padding='SAME')
            biases = tf.Variable(
        ...
        with tf.name_scope('conv4_1') as scope:
            kernel = tf.Variable(
                tf.truncated_normal([3, 3, 256, 512], dtype=tf.float32,
                                    stddev=1e-1), name='weights')
            conv = tf.nn.conv2d(pool3, kernel, [1, 1, 1, 1], padding='SAME')
            biases = tf.Variable(
        ...
        with tf.name_scope('conv4_2') as scope:
            kernel = tf.Variable(
                tf.truncated_normal([3, 3, 512, 512], dtype=tf.float32,
                                    stddev=1e-1), name='weights')
            conv = tf.nn.conv2d(conv4_1, kernel, [1, 1, 1, 1], padding='SAME')
            biases = tf.Variable(
                tf.constant(0.0, shape=[512], dtype=tf.float32),
        ...
        with tf.variable_scope('local4') as scope:
            reshape1 = tf.reshape(pool5, [pool5.get_shape()[0].value, -1])
            ...
            weights = _variable_with_weight_decay('weights', shape=[dim, 4000],
                                                  stddev=0.01, wd=0.0)
            biases = _variable_on_cpu('biases', [4000],
                                      tf.constant_initializer(0.1))
