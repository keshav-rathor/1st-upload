...
def eval_once(saver, prob_op, gt_op, mask=None):
  """Run Eval once.
...
def evaluate(data_dir='', ids=None, fcout=False):
  ...
  with tf.Graph().as_default() as g:
    ...
    coefs, labels = model.inputs(eval_data=eval_data)
    # if eval_data:
    #   mask = np.array([0, 1, 2, 3, 4, 10, 11, 13, 14])

    # Build a Graph that computes the logits predictions from the
    # inference model.
    logits, _, _ = model.inference(coefs)
    ...
    while True:
      eval_once(saver, probs, labels, mask)
      if FLAGS.run_once:
        break
