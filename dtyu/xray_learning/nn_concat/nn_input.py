...
def distorted_inputs(data_dir, batch_size, shuffle=True):
  ...
  # 1D transform
  log_oned = tf.log(1 + read_oned) / np.log(2**16)
  #normalized_oned = (log_oned - tf.reduce_min(log_oned)) / (tf.reduce_max(log_oned) - tf.reduce_min(log_oned))
  ...
  return _generate_image_and_label_batch(read_input.key, float_image, log_oned, read_input.label,
                                         min_queue_examples, batch_size,
                                         shuffle=shuffle)


def inputs(eval_data, data_dir, batch_size, num_threads=16, input_override=None, num_examples=0):
  """
  1D curve added.
