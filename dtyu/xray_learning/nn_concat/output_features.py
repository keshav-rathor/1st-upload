...
def eval_once(saver, keys_op, fc1_op, fc2_op, logits, oneds_op, labels):
  ...
  with tf.Session() as sess:
    ...
    #     gt_all = gt_all[:, mask]
      ...
      oned_binary_path = '../xray_data/synthetic_output_features/'
...
def output_features():
  ...
  with tf.Graph().as_default() as g:
    ...
    if FLAGS.eval_data == 'train':
      keys_op, images, oneds_op, labels = model.inputs('train', num_threads=1,
          input_override=[os.path.join('../xray_data/synthetic_data_binary', 'stagged-batch-%d.bin' % i) for i in range(19)],
          num_examples=95000)
    elif FLAGS.eval_data == 'val':
      keys_op, images, oneds_op, labels = model.inputs(False)
    ...
    # inference model.
    fc1_op, fc2_op, logits, _ = model.inference(images, oneds_op, dropout=True)
