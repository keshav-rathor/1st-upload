...
def read_data(filename_queue):
  ...
  result.coef = tf.slice(float_tensor, [int(label_bytes / 4)],
                  [coef_height * coef_width * coef_depth])
  ...
  result.image = tf.image.resize_images(image, [128, 128])
  # result.image = tf.slice(uint_tensor, [label_bytes + coef_bytes],
  #                         [result.height * result.width * result.depth])
...
def distorted_inputs(file_list, num_examples, batch_size, shuffle=True, num_threads=16, nodistort=False):
  """
  1D curve added.
  ...
  normalized_coef = tf.nn.l2_normalize(read_input.coef, 0)
...
def inputs(file_list, num_examples, batch_size, num_threads=1):
  """
  1D curve added.
