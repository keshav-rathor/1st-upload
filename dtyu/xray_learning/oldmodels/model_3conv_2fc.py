...
def rebalanced_distorted_inputs(pos_sample_dir):
  ...
  coefs2, images2, labels2 = nn_input.distorted_inputs(data_dir=pos_sample_dir,
                                                    batch_size=int(FLAGS.batch_size / 2),
                                                    input_type='aug')
  return tf.concat(axis=0, values=[coefs1, coefs2]), \
         tf.concat(axis=0, values=[images1, images2]), \
         tf.concat(axis=0, values=[labels1, labels2])
  ...
  if architecture == 'cnn':
    ...
    with tf.variable_scope('conv1') as scope:
      ...
      for height_idx in range(0, filter_layer1_height):
        ...
        filter_layer1[height_idx] = tf.transpose(tf.concat(axis=0, values=tmp))
    ...
    with tf.variable_scope('fconv1') as scope:
      ...
      for height_idx in range(0, ffilter_layer1_height):
        ...
        tmp = tf.split(axis=0, num_or_size_splits=ffilter_layer1_width, value=ffilter_layer1[height_idx])
    ...
    with tf.variable_scope('fconv3') as scope:
      weights = _variable_with_weight_decay('weights', shape=[3,3,128,128], stddev=0.01, wd=0.0)
      biases = _variable_on_cpu('biases', [128], tf.constant_initializer(0.1))
      conv = tf.nn.conv2d(fconv2, weights, [1, 1, 1, 1], padding='SAME')
      bias = tf.nn.bias_add(conv, biases)
    ...
    with tf.variable_scope('ffc1') as scope:
      reshape = tf.reshape(fconv3, [fconv3.get_shape()[0].value, -1])
    ...
    with tf.variable_scope('fconv2') as scope:
      weights = _variable_with_weight_decay('weights', shape=[3, 3, 64, 128],
                                            stddev=0.01, wd=0.0)
      biases = _variable_on_cpu('biases', [128], tf.constant_initializer(0.1))
      conv = tf.nn.conv2d(fconv1, weights, [1, 1, 1, 1], padding='SAME')
      bias = tf.nn.bias_add(conv, biases)
    ...
    with tf.variable_scope('local4') as scope:
      ...
      reshape2 = tf.reshape(fconv2, [fconv2.get_shape()[0].value, -1])
